...
LOGLIMIT="-m limit --limit 10000/min"
...
##### 10: Log client signons - string is at the end of the packet
...
##### 9: Grab client signon packets to whitelist
${ipt_pre_raw} -p udp -i ${defaultin} ${COMMENT} ${ports}                   \
    ${RULE_FILTER} '|3030303030303030303000|'                               \
    -m length --length 48 --from 26 --to 48                                 \
    -j SET --add-set signed_on src,dst --timeout 120

##### 8: Log client signoffs - string is at the end of the packet
${ipt_pre_raw} -p udp -i ${defaultin} ${COMMENT} ${ports}                   \
...
##### 7: Grab client signoff packets to unwhitelist clients
...
########## Ignore signed on and trusted users for these next two rules
RULE_FILTER="-m hashlimit --hashlimit-name speedlimit --hashlimit-mode srcip,dstport --hashlimit-above 3/sec --hashlimit-burst 5"
...
##### 4: Allow signed on gamers to play - only whitelists ip to destip / dport :D
${ipt_pre_raw} -p udp -i ${defaultin} ${COMMENT}                            \
    -m set --match-set signed_on src,dst                                    \
    -j ACCEPT

##### 3: Reset timeout whenever we get some data from this client
...
if [[ -f "$trustedhosts" ]]; then
    ...
    do
        #${ipt_pre_raw} -p udp ${COMMENT} -s "$line"                         \
        #    -j SET --add-set permatrusted src
        echo "$line added to trusted hosts"
    done < "$trustedhosts"
fi


##### 1: Disable conntracking on our srcds ports
